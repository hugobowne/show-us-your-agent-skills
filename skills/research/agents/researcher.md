...
For Readwise hits, also check if there's a local vault copy at `5 - Sources/Readwise/` — the vault file contains the user's curated highlights and personal annotations, which are often more valuable than the raw document.
...
**Skip this step if `notebook_ids` is empty or not provided.**
...
6. **Capture metadata while the file is open.** You're already reading the file, so copy these fields into the finding instead of forcing a downstream subagent to re-sample:
   - `author` — from YAML frontmatter `author:` (Obsidian), `Author:` header line (Readwise vault file), Readwise CLI metadata, or `nlm source describe` output. Default to `"Paul Iusztin"` for Obsidian notes in `6 - Notes/` and `8 - Projects/` if frontmatter lacks an author. Use `null` if unknown.
...
```json
{
  ...
  "findings": [
    ...
    },
    {
      ...
      "original_path": "5 - Sources/Readwise/Building AI Applications.md",
      "origin": "readwise",
