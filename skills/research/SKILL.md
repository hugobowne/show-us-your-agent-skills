...
Use the same locator logic as Step 0 (path provided / topic slug / scan `<working_memory_dir>` / ask if multiple).
...
```bash
uv run python .claude/skills/research/scripts/build_index_md.py --research-dir "<research_dir>"
```
...
3. **Readwise references**: If the user references a specific Readwise source by name, search for it via the MCP tool.
4. **NotebookLM references** (e.g., `nlm://notebook/<id>`, a notebook name, or "my NotebookLM notebook on X"): These reference an entire notebook. Do not treat the notebook itself as a seed source — instead, note the notebook ID for the NLM discovery step (Step 3b) so it's always included in the search. Individual sources within it will be discovered during the research rounds.
5. **GitHub repositories** (e.g., `https://github.com/owner/repo` or `.../tree/<branch>`): Process via the GitHub pipeline below (Step 1a). Produces one `ARCHITECTURE.md` always and one `<module>.md` per targeted module. GitHub never participates in research rounds — it is always seed-only.
6. **Local PDFs** (e.g., a path ending in `.pdf` from the user's filesystem, including paths inside the vault like `3 - Media/some-paper.pdf`): Treat as a first-class seed. The actual extraction happens in Step 6.2 via `scripts/extract_pdf.py` — at this stage just record the seed entry with `origin: "pdf"`, `original_path: pdf://<basename>`, `source_url: null`, and a placeholder `summary` (the user's framing if they provided one, else empty — the source_writer will fill it in from extracted text in Step 6.3). Store the absolute path on the seed entry as `local_pdf_path` so Step 6.2 knows where to extract from.
7. **Web PDFs** (a URL ending in `.pdf`): Same treatment as local PDFs, but Step 6.2 will download to `raw/assets/<slug>/original.pdf` first via `httpx`, then extract. If the `httpx` download is blocked (403 / bot wall / CAPTCHA), retry the download through Bright Data (`bdata scrape "<url>" -o "raw/assets/<slug>/original.pdf"`, see `/brightdata-cli`) before giving up.
...
When the brain dump contains a GitHub repo URL, process it BEFORE writing `seeds.json`. The GitHub pipeline never clones *into* the research directory; clones go to a reusable `.github-cache/` placed **as a sibling of the research dir** (i.e. in the research dir's parent — e.g. for `8 - Projects/My Project/research-<slug>/` the cache lands at `8 - Projects/My Project/.github-cache/`). This keeps each project's reusable clones next to its own folder. Only curated spec docs land in the final research dir.
...
1. **Parse targets** — run the parser over the brain dump AND any markdown files the user referenced (e.g., an outline.md). The parser extracts repo-relative file paths + line ranges and groups them by parent directory (one "module" per dir):
   ```bash
   uv run python <skill_dir>/scripts/github_parse_targets.py \
     --repo "<repo_url>" \
...
2. **Shallow-clone** the repo into the cache. Pass `--research-dir` so the cache lands as a sibling of the research dir (its parent), not under working memory:
   ```bash
   uv run python <skill_dir>/scripts/github_clone.py \
     --repo "<repo_url>" \
...
1. **Check authentication**: Run `nlm login --check`. If it fails, log a loud warning to the user that NotebookLM will be skipped for this run, set `notebook_ids = []`, and continue. Do NOT silently skip — the user needs to know NLM was offline so they can decide whether to re-run after fixing auth.
...
1. **Deduplicate** the round's subagent outputs via script:
   ```bash
   uv run python .claude/skills/research/scripts/dedup_findings.py \
...
1. **Compute the research-dir path**. Default: `<working_memory_dir>/research-<topic-slug>/`. Override if the user specified a different output path. Create the v4 layout up front:
   ```bash
...
2. **Spawn the Builder Subagent**. Read `agents/builder.md` and pass it as the subagent prompt, along with:
   ...
   - `skill_dir`: `.claude/skills/research` (absolute path)
...
3. **The builder returns**:
   ...
   **Layer 2 (`uri_highlights`) rule unchanged**: populated only when the source carries manually user-curated highlights (Readwise highlights synced into the vault at `5 - Sources/Readwise/`, and raw Readwise documents whose vault copy holds the user's highlights). For everything else, `uri_highlights` is `null` and the full content lives at `uri_full` (Layer 3).
...
1. **PDFs** — for any source where `origin == "pdf"` (user-dropped) or where the original is a `.pdf` URL, the Builder will have already invoked `extract_pdf.py` to extract markdown + images and preserve the original at `raw/assets/<slug>/original.pdf`. If somehow not done, run it now:
   ```bash
   uv run python <skill_dir>/scripts/extract_pdf.py \
...
2. **Images** — for every successful raw markdown file, download referenced images to local assets and rewrite refs:
   ```bash
   uv run python <skill_dir>/scripts/download_assets.py \
...
2. **Run `build_index_yaml.py`** to emit the canonical index:
   ...
   uv run python <skill_dir>/scripts/build_index_yaml.py \
...
3. **Run `build_index_md.py`** to regenerate the Obsidian-readable view:
   ```bash
   uv run python <skill_dir>/scripts/build_index_md.py \
     --research-dir "<research_dir>"
   ```
...
**Search strategy — Obsidian**: Use the `obsidian` CLI for search operations: `obsidian search query="<terms>" limit=20`. Once you have the file paths from search results, read the files directly using the Read tool. See the `obsidian-cli` skill for full command reference.

**Search strategy — Readwise**: Use the `readwise` CLI (not the MCP tool). For the full command reference, access the `readwise-cli` skill. Readwise splits into two areas and research must cover both:
